...
@Entity
@Table(schema="ALUMNO_AAG", name = "Productos",uniqueConstraints = {@UniqueConstraint(columnNames = {"id_producto"})})
@SecondaryTables({
...
public class Producto implements Serializable, Auxiliar{ 
	...
	public boolean isValidInsert() throws DomainException {
		if(Validator.cumpleIdproduc(id_producto) && Validator.cumpleLongitudMax(pro_descripcion,MAX_DESC_COR)&&
				pro_precio!=0 && Validator.isVacio(pro_uniVenta) && id_pais!=0)
			return true;
		else {
			throw new DomainException(Messagesmessages.PROERR_013);
		}
	}
	@JsonIgnore
	@Transient
